...
// doesn't matter, only that a valid image round-trips.
...
// (not a passthrough) when the real call succeeds, gated on real infra.
describe.skipIf(!infraReady)("addendum 3: prettify (llamada real)", () => {
...
describe("addendum 3: prettify (degradacion sin romper el flujo)", () => {
  it("si el modelo no devuelve imagen, enhancePhoto lanza en vez de devolver basura silenciosa", async () => {
    // No credentials / unreachable in this environment -> the SDK call
    // itself throws before files are ever inspected, which is exactly the
    // failure the API route's try/catch is built to catch without losing
    // the item's original photo.
